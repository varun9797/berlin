    ...
    if (!user) {
        ...
        if (!isMatch) {
            ...
            const token = jwt.sign(
                { username: user.username, userId: user._id }, process.env.JWT_SECRET || "default", { expiresIn: '30d' });
            // res.setHeader("Authorization", `Bearer ${token}`);
            // res.setHeader('Access-Control-Expose-Headers', 'Authorization'); // optional if using `exposedHeaders` in cors

            const refreshToken = jwt.sign(
                { username: user.username, userId: user._id }, process.env.JWT_REFRESH_SECRET || "default", { expiresIn: '30d' });
